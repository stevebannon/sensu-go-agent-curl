...
if [ "$(echo "${cpu_usage} > ${CRIT}" | bc)" -eq 1 ]; then
  echo "CPU CRITICAL - ${cpu_usage}% is greater than critical point.[${CRIT}] | cpu=${cpu_usage}"
  exit 2
fi

if [ "$(echo "${cpu_usage} > ${WARN}" | bc)" -eq 1 ]; then
  echo "CPU WARNING - ${cpu_usage}% is greater than warning point.[${WARN}] | cpu=${cpu_usage}"
  exit 1
fi

echo "CPU OK - Usage:${cpu_usage} | cpu=${cpu_usage}"
exit 0
